...
export default class Gomoku extends React.Component {
	constructor(){
		...
		this.state = {
			board: Array(this.size**2 + 1).fill(false),
	...
	calculateWinner(board, position){
		// TODO: Fix the bug where player token lines that wrap to a new line are counted as a win.
		let lines = [];
		let winner = null;
		...
		for (let i = -this.rowLength + 1; i <= 0; i++) {
			...
			for (let j = 0; j < this.rowLength; j++) {
				...
				posDiagonal.push(position + (this.size * x + x));
				negDiagonal.push(position + (-this.size * x + x));
			}
		...
		winner = lines.some(function(line) {
			return line.every(function(p, i, a) {
				if(i > 0) { // Fixing a bug where lines that wrap to new lines were valid.
					const curX = Math.floor(p % this.size);
					const prevX = Math.floor(a[i-1] % this.size);
					const xDelta = curX - prevX;

					if (Math.abs(xDelta) > 1) return false;
				}
